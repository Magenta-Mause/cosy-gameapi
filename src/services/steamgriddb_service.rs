...
impl SteamgriddbService {
    pub fn new(sg_client: Arc<steamgriddb_api::client::Client>, req_client: Arc<ReqwestClient>, base_url: String) -> Self {
        Self { sg_client, req_client, base_url }
    }

    pub async fn fetch_assets_by_game_id(&self, game_id: usize) -> Result<Vec<steamgriddb_api::images::Image>, Box<dyn Error>> {
        self.sg_client.get_images_for_id(game_id, &steamgriddb_api::QueryType::Grid(None)).await
    }
    ...
    pub async fn get_first_logo_by_game_id(&self, game_id: usize) -> Result<Option<String>, Box<dyn Error>> {
        ...
        let logos_resp = self.req_client.get(logos_url).query(&[("limit", 1)]).send().await?;
    ...
    pub async fn get_first_hero_by_game_id(&self, game_id: usize) -> Result<Option<String>, Box<dyn Error>> {
        ...
        let heroes_resp = self.req_client.get(heroes_url).query(&[("limit", 1)]).send().await?;
